...
function CN_StartTTGPT_Prompt() {
	...
	if (TTGPT_PROMPT_ALREADY_SHOWN == "shown4") {
		...
		if (confirm("LightRein|MackTradingMatrix - COMPATIBILITY INFO - PLEASE NOTE:\n" +
...
// Start LightRein|MackTradingMatrix (Start button)
...
function CN_InitScript() {
		...
		"<div style='position: fixed; top: 50px; right: 16px; display: inline-block; " +
			...
			"<div style='padding: 4px 40px; border-bottom: 1px solid grey;'>" +
				"<a href='https://github.com/C-Nedelcu/talk-to-chatgpt' " +
					...
					"target=_blank title='Visit project website'>LightRein|MackTradingMatrix<br />" +
	...
	jQuery(document).on('keydown', function (e) {
		...
		if (e.altKey && e.shiftKey && e.which === 83) {
			console.log('[KEY] ALT+SHIFT+S pressed, starting LightRein|MackTradingMatrix');
			CN_StartTTGPT();
		}
...
function CN_OnSettingsIconClick() {
	...
	rows += "<tr><td style='white-space: nowrap'>Keyboard shortcuts:</td><td><ul>" +
		"<li>ALT+SHIFT+S: <u>S</u>tart LightRein|MackTradingMatrix</li>" +
	...
	// Add donations frame
	var donations = "<br/><h2>Support the project</h2><p style='font-size: 15px; margin-top: 15px;'>Are you enjoying LightRein|MackTradingMatrix and want me to continue improving it? \n" +
		"\t\t<b>You can help by making a donation to the project.</b> \n" +
		"\t\tPlease click the button below to proceed.</p><br />\n" +
		"\t\t<center><a target=_blank href='https://www.paypal.com/donate/?business=BZ43BM7XSSKKW&no_recurring=0&item_name=Are+you+enjoying+LightRein|MackTradingMatrix?+If+so%2C+consider+making+a+donation+to+keep+the+project+going%2C+and+I%27ll+continue+improving+it%21&currency_code=EUR'>\n" +
	...
	jQuery("body").append("<div style='background: rgba(0,0,0,0.8); position: absolute; overflow-y: auto; top: 0; right: 0; left: 0; bottom: 0; z-index: 999999; padding: 20px; color: white; font-size: 13px;' id='TTGPTSettingsArea'>" +
		"<div style='width: 600px; margin-left: auto; margin-right: auto; overflow-y: auto;'><h1>⚙️ LightRein|MackTradingMatrix settings</h1>"+desc+rows+donations+"</div></div>");
...
(function () {
	...
	setTimeout(function() {
		typeof jQuery == "undefined" ?
			alert("[LightRein|MackTradingMatrix] Sorry, but jQuery was not able to load. The script cannot run. Try using Google Chrome or Edge on Windows 11") :
			CN_CheckCorrectPage();
	}, 500);
